# exemplo: python app_get_img_goes.py -y 2024 -s 1 -e 365 -d 256 -p 10 -o images
#!/usr/bin/python3
...
def get_s3_keys(bucket, prefix = ''):
    ...
    # """ Você devera criar o seu proprio usuario e secret key e ingressar com estes dados abaixo.


    aws_access_key_id  = 'bla..bla..bla...bla'  /// Crie aqui seu acesso no AWS
    aws_secret_access_key = 'bla.bla.bla.blc....'  /// Crie aqui sua propria Secret Key
    print(prefix)
...
def converter_dia_do_ano_para_data(dia_do_ano, ano_base):
    # Criar uma data base no ano atual
    
    #data_base = datetime(datetime.now().year, 1, 1)
    
    data_base = datetime(ano_base, 1, 1)
    
    # Adicionar o número de dias ao dia base
    data_final = data_base + timedelta(days=dia_do_ano - 1)
    
    # Formatando a data como dd/mm
    return data_final.strftime('%d-%m')
...
def main():
    ...
    parser.add_argument('-d', "--dimension",
        type=int,
        default=256,
        help='R|Dimension (Resolution) of the image to be saved in the disk (Default: %(default)s)')
        ...
        with trange(((args.endday - args.startday)+1) * len(args.period), initial=0, desc='Progress  ', leave=False) as pbTotal:
            while args.startday <= args.endday:
                for hour in args.period:
                    getimage(args.year, args.startday, hour, args.outputpath, args.channel, args.crop, args.dimension)
                    pbTotal.update(1)
...
def getimage(year, day, hour, outputpath, channel, crop, dimension):
    try:
       ...
       # outputpath = outputpath + '/' + getNameChannel(channel) + '/' + str(year).zfill(4) + '/' + str(day).zfill(3) 
        ...
        outputpath = outputpath + '/' + str(year).zfill(4) 
        ...
        with open(outputpath + '/pure/' + file_name + '.nc', 'rb') as in_file:
            ...
            #plt.imshow(DS.Rad[left:right, top:bottom], cmap='color')

            plt.axis('off')
            
            file_name1 = converter_dia_do_ano_para_data(day, year) + '-' + str(year) + '_' + str(hour)
            
            plt.savefig(outputpath + '/' + file_name1 + '_.png', dpi=300, facecolor='w', edgecolor='w', bbox_inches='tight')
            plt.close(fig)
        os.remove(outputpath + '/pure/' + file_name + '.nc')
        image = Image.open(outputpath + '/' + file_name1 + '_.png')
        ...
        #nova resolução
        new_image = image.resize((dimension, dimension))        
        newfilename=outputpath + '/' + file_name1 + '.png'
        print(newfilename)               
        
        new_image.save(newfilename)
        image.close()
        os.remove(outputpath + '/' + file_name1 + '_.png')
        print('image save :'+newfilename)

        # Abra a imagem em formato PNG
        jpg_file=outputpath + '/' + file_name1 + '.jpeg'
        imagem = cv2.imread(newfilename)  # Substitua pelo caminho correto
        cv2.imwrite(jpg_file, imagem)

        #Apagando o arquivo original (PNG)
        os.remove(outputpath + '/' + file_name1 + '.png')
